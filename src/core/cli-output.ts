...
import { firstString, isRecord } from './value.js';

export function printHelp(role: Role, commands: string[]): void {
  const recommended = getRecommendedCommands(role, commands);
  process.stdout.write([
    `zentao CLI ${CLI_VERSION}`,
    '',
    `当前 role: ${role}`,
    ...
    '',
    '用法：',
    '  zentao [--role full|dev|pm|qa] <command> [options]',
    '  zentao [--role=full|dev|pm|qa] <command> [options]',
    '',
    '快速开始：',
    '  zentao list                  查看全部可用命令（推荐）',
    ...
    '',
    '安装/更新：',
    '  zentao install               一键安装 CLI + Skill',
    '  zentao update                更新到最新版本',
...
export function printCommandList(role: Role, commandNames: string[], builtinCommandNames: string[]): void {
