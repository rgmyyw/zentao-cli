...
调用本 CLI 的列表型命令（`getMyTasks` / `getMyBugs` / `getMyTaskStatistics` / `getMyBugStatistics` / `getProductBugs` / `getProductStories` / `getMyWeeklyActivity` 等）时：
...
- 只有当用户明确要求"逐条展示"/"按某字段筛选"时，才遍历 `items`，且不要再做二次清洗。
- 不要把 `product` / `productName` / `productTitle` 当成同一个字段；同理 `execution` / `executionName` / `executionTitle` / `sprintName` —— 全部以 `items[i].xxxName` 优先，必要时回退到 `xxx`。
