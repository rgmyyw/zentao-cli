...
1. 普通禅道产品 Bug
2. 市场 / 售后 / 客户反馈来源的线上 Bug
3. 测试或开发自己在线上发现、记录在测试相关模块下的 Bug

如果用户表达了以下任一意图，先按“线上 Bug 来源分流”判断来源：
...
这类场景里，用户说的 `<产品名>` 通常是真实业务产品 / 模块名，不一定是禅道产品名。
...
只有当用户明确说“查某个禅道产品下的 Bug”，或上下文明显不是线上 / 生产 / 客户反馈 / 测试开发自发现问题时，才按普通产品 Bug 查询。

### 线上 Bug 来源分流

默认口径：非测试 / 非开发发现的线上、生产、客户反馈、售后反馈问题，优先查禅道产品 **`市场和售后问题跟踪`**，再通过 Bug 模块区分真实业务产品。

例外口径：如果上下文明确说明问题是测试或开发自己在线上发现的，通常查禅道产品 **`测试`** 及其业务模块体系，例如 `测试 > 云镜`，再用模块名、模块路径或模块别名过滤。
...
2. 找到名称为 `测试` 的产品
3. 用该产品 ID 调 `getProductBugs`
4. 用 `module` / `moduleName` / `modulePath` / `path` 匹配业务模块，例如 `云镜` 或 `yj`

输出必须说明：

- 查询的是哪个禅道产品：默认是 `市场和售后问题跟踪`；测试 / 开发自发现时是 `测试`
- 真实业务产品是通过模块匹配出来的
- 匹配到的模块名 / 模块路径
- 查询页数和候选 Bug 数
