...
- **MCP server version 改为从 CLI_VERSION 读取**：`mcp.ts` 原硬编码 `version: '1.0.0'`（已过期），改为 `version: CLI_VERSION`，与 CLI 本身版本一致。
...
- **密码输入改为明文**：`install.ts` 的 `askPassword` 不再做掩码隐藏（`*` 回显），直接复用 `ask` 明文输入，密码输入时可见。移除 `_writeToOutput` hook 与 `stdoutMuted` 逻辑。

## 1.1.4 - 2026-07-28

### 修复

- **版本号显示修复**：`version.ts` 改为从 `package.json` 动态读取版本号，避免 bump 后 CLI 显示旧版本号。
