...
describe('runDailyUpdateProbe', () => {
  it('notifies when cache has a newer version', async () => {
    ...
    vi.doMock('node:fs/promises', () => ({
      ...
      readFile: vi.fn(async () => JSON.stringify({ lastCheckedDate: '2026-01-01', latestVersion: '0.1.28', currentVersion: '0.1.24' })),
    ...
    expect(write).toHaveBeenCalledWith(expect.stringContaining('检测到 zentao CLI 新版本 0.1.28（当前 0.1.27）。'));
